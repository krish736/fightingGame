...
const updateGame = (p1,p2,gameState) => {
  // Update the DOM with the names and the latest health of 
...
let player2=new Player('kartik',100,10)
...
document.addEventListener('keydown', function(e) {
  ...
  if(e.key=='a' && 100 > p1.health >0 && game.isOver==false){
    p1.heal(p1)
...
document.addEventListener('keydown', function(e) {
  // if you press a AND the player health is greater than 0 AND isOver is still false then heal()
  if(e.key=='l' && 100 > p2.health >0 && game.isOver==false){
    p2.heal(p2)
    // After healing then play heal sound
